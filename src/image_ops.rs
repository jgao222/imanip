...
pub fn identity_image(image: Image) -> Image {
    image.apply_to_channels(&|chan: &ImageChannel| apply_filter(chan, &kernels::IDENTITY1))
}

pub fn simple_gaussian_blur(image: Image) -> Image {
    image.apply_to_channels(&|chan| apply_filter(chan, &kernels::GAUSSIAN_SIMPLE))
}
...
pub fn complex_gaussian_blur(image: Image) -> Image {
    ...
    let img = image.apply_to_channels(&|chan| apply_filter(chan, &kernels::GAUSSIAN_X));
    img.apply_to_channels(&|chan| apply_filter(chan, &kernels::GAUSSIAN_Y))
}

/// sharpens an image by subtracting its laplacian from itself
pub fn sharpen_image(image: Image) -> Image {
    let img_lap: Vec<u8> = image
        .apply_to_channels(&|chan| apply_filter(chan, &kernels::LAPLACIAN))
        .into();
    ...
    Image::new(
        ...
        image.height,
        image.color_type,
        &subtract_images(&image_bytes, &img_lap),
    )
}
