...
fn main() {
    ...
    let output_image = match args[1].chars().collect::<Vec<char>>()[1] {
        ...
        's' => sharpen_image(bytes, &image_info),
        'u' => todo!(),
        _ => {
    ...
    writer
        .write_image_data(&output_image.bytes)
        .expect("should have successfully written transformed image data to file");
}

fn apply_to_channels(
    image: &[u8],
    image_info: &OutputInfo,
    apply_func: &dyn Fn(&[u8], &OutputInfo) -> Image,
) -> Image {
...
fn identity_image(image: &[u8], image_info: &OutputInfo) -> Image {
    apply_to_channels(image, image_info, &|chan: &[u8], info: &OutputInfo| {
        apply_filter(chan, info, &kernels::IDENTITY1)
    })
}

fn simple_gaussian_blur(image: &[u8], image_info: &OutputInfo) -> Image {
    apply_to_channels(image, image_info, &|chan, info| {
        apply_filter(chan, info, &kernels::GAUSSIAN_SIMPLE)
    })
}
...
fn complex_gaussian_blur(image: &[u8], image_info: &OutputInfo) -> Image {
    ...
    let img = apply_to_channels(image, image_info, &|chan, info| {
        apply_filter(chan, info, &kernels::GAUSSIAN_X)
    });
    apply_to_channels(&img.bytes, image_info, &|chan, info| {
        apply_filter(chan, info, &kernels::GAUSSIAN_Y)
    })
}

/// sharpens an image by subtracting its laplacian from itself
fn sharpen_image(image: &[u8], image_info: &OutputInfo) -> Image {
    let img_lap = apply_to_channels(image, image_info, &|chan, info| {
        apply_filter(chan, info, &kernels::LAPLACIAN)
    });
    Image {
        width: image_info.width,
        height: image_info.height,
        bytes: subtract_images(image, &img_lap.bytes),
    }
}
...
fn subtract_images(image1: &[u8], image2: &[u8]) -> Vec<u8> {
    image1
        .iter()
        .zip(image2.iter())
...
#[allow(unused)]
fn add_images(image1: &[u8], image2: &[u8]) -> Vec<u8> {
    image1
        .iter()
        .zip(image2.iter())
